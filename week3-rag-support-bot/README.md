...
All of the above is implemented in rag_pipeline.py (core RAG logic) and app.py (chat UI).
...
- rag_pipeline.py - core RAG pipeline: ingestion, embeddings, retrieval, escalation logic; also runnable standalone from the terminal
- knowledge_base folder - the real Nexariza AI content the bot is grounded on
...
On first run, the app builds a local vector database from the knowledge_base folder (downloads a small embedding model once, then works fully offline).
